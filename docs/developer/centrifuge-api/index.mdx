...
Holdings track pool-level asset positions, providing a unified view of what each pool owns. Each holding represents a specific asset position with its quantity and valuation.

| Filter | Type | Description |
|--------|------|-------------|
| `poolId` | `BigInt` | Pool the holding belongs to |
| `tokenId` | `String` | Share class token ID |
| `isLiability` | `Boolean` | Whether the holding is a liability |
