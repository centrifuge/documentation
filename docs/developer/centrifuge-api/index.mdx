...
import RESTExplorer from '@site/src/components/RESTExplorer';
...
<GraphQLExplorer
  query={`query GetTokenYields {
  tokenSnapshots(
    where: { tokenId: "0x00010000000000060000000000000001" }
    orderBy: "timestamp"
    orderDirection: "desc"
    limit: 1
  ) {
...
| Field | Description |
|-------|-------------|
| `yieldSinceInception` | Total return since the token was first issued |
| `yieldYtd` | Year-to-date return |
| `yieldTtm` | Trailing twelve months return |
| `yield7d` / `yield30d` | Simple return over the trailing 7 / 30 days |
| `yield7d360` / `yield7d365` | 7-day return annualized on a 360 / 365 day basis |
| `yield30d360` | 30-day return annualized on a 360 day basis |
| `yield30dComp360` / `yield30dComp365` | 30-day return compounded and annualized on a 360 / 365 day basis |

| Filter | Type | Description |
|--------|------|-------------|
| `tokenId` | `String` | Share class token ID |
| `timestamp` | `BigInt` | Snapshot timestamp (supports `_gt`, `_lt`, `_gte`, `_lte`) |
...
<RESTExplorer url="https://api.centrifuge.io/tokens/0xaaa0008c8cf3a7dca931adaf04336a5d808c82cc/price">
...
<RESTExplorer url="https://api.centrifuge.io/tokens/0xaaa0008c8cf3a7dca931adaf04336a5d808c82cc/total-issuance">
