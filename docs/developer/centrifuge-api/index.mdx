...
| `poolId` | `String` | Pool the vault belongs to |
...
Holdings track pool-level asset positions across different blockchains, providing a unified view of what each pool owns. Each holding contains accounts that represent specific asset positions.

| Filter | Type | Description |
|--------|------|-------------|
| `poolId` | `String` | Pool the holding belongs to |
...
| Filter | Type | Description |
|--------|------|-------------|
| `accountId` | `String` | Investor wallet address |
| `chainId` | `Int` | Network chain ID |
| `type` | `String` | Transaction type (e.g. `INVEST`, `REDEEM`, `CLAIM`) |
| `tokenId` | `String` | Share class token ID |
