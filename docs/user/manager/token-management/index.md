...
Each share class of the pool is represented by a token. The token is what investors hold, what carries the price, and what moves. It can be distributed to multiple chains and, depending on how the manager configures it, traded or sold on DeFi protocols.
...
1. The manager opens the share class to update it.
...
Whether the token launches on one network or on five, it is one step, with no per-chain deployment project. Each deployment remains part of the same share class, and more networks can be added later as distribution grows (see [Distribution](/user/manager/distribution)).
