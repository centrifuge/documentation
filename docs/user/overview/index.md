...
Centrifuge is the open infrastructure for onchain asset management. Financial products are represented and operated onchain through pools: managers run product operations, investors access them from their own wallets, and curators design and oversee onchain allocation strategies. Centrifuge provides the rails, and each product remains the responsibility of its manager.
...
**Managers** structure a financial product and operate it onchain through the Management app: issuance, investor access, pricing, orders, distribution and reporting, from one interface and without writing code. See the [manager guide](/user/manager).
...
These roles can come together in the same product: the manager operates the pool, investors enter and exit through its vaults and, where the strategy includes onchain allocation, a curator manages how its capital is deployed.

<img
  src={rolesTogether}
/>
> One product, three roles: the manager operates the pool, investors enter and exit through its vaults, and the curator, as the product's portfolio manager, allocates its capital across onchain venues.
