...
These controls are scoped per token, not per pool: each share class carries its own restriction profile and its own whitelist, so two classes of the same product can follow different distribution models. And they are not fixed at launch — permissions can be updated from the app as the offering evolves. Who is on the whitelist is a separate, day-to-day operation: see [Investor management](#investor-management).
