...
Beyond whitelisting, each share token supports controls that can be operated throughout the product's life:
...
These controls can be managed centrally from the pool's hub. For example, a batch of investors can be whitelisted across the selected networks in one action, avoiding separate updates on each network.
